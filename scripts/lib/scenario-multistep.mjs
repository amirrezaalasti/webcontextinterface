...
function generatedStandardFlow(meta, primarySel) {
  return [
...
export function addMultiStepTasks(meta, opts = {}) {
  ...
  if (legacy) {
    ...
    taskA = buildTaskObject(
      ...
      standardFromMeta.length ? standardFromMeta : generatedStandardFlow(meta, primarySel),
      wciFromMeta.length ? wciFromMeta : generatedWciFlow(meta),
    ...
    taskB = buildTaskObject(
      ...
      [
        ...
        { step: `Execute correct action ${primarySel}`, type: 'act' },
        { step: 'Verify goal is complete', type: 'verify' },
      ],
      [
        { step: 'Use WCI scope to identify decoy ids up-front', type: 'read' },
        { step: `Execute ${meta.groundTruth?.wciNodeId} directly`, type: 'act' },
    ...
    taskA = buildTaskObject(
      ...
      generatedStandardFlow(meta, primarySel),
      generatedWciFlow(meta),
    ...
    taskB = buildTaskObject(
      ...
      [
        ...
        { step: `Execute primary selector ${primarySel}`, type: 'act' },
      ...
      [
        ...
        { step: `Click ${meta.groundTruth?.wciNodeId}`, type: 'act' },
