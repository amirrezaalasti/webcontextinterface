...
| **Annotation cost, drift, or errors** | WCI paths assume annotations are already **correct and complete**. The benchmark does not score mis-labeling, drift, or partial coverage. Publisher effort (one page per site): **~295 ± 125 DOM elements per page**, **~106 ± 26 WCI nodes** (~**39% ± 6%** of that page), **~620 ± 155 labels** (mean ± σ; medians ~257 / ~105 / ~39% / ~598). Per-site delta and z-score vs suite mean: `benchmark-info.json` → `scenarios.*.vsSuiteMean`. |
